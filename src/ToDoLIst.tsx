...
//-------функция удаления полей-------
    ...
    const tasksList = props.tasks.map((item) => {
        ...
        <li className={item.isDone? s.isDoneTask: ''}>
            ...
            <Button className={''} name = 'x' callBack={()=>{deleteHandler(props.toDoListId, item.id)}}/>
        </li>
    )
    }
    )
...
//-------функция удаления списка-------
    ...
    return (
        <div>
            ...
            <div>
                <Input toDoListId={props.toDoListId} error={classNameError} userInput={userInput} setUserInput = {setUserInput} addTask={addTask} setError={setError}
                />
            ...
            <div>
                <Button className={props.filter == 'all'?'activeButton':''} name={'All'} callBack ={()=>onClickFilterHandler('all')} />

                <Button className={props.filter == 'active'?'activeButton':''} name={'Active'} callBack ={()=>onClickFilterHandler('active')} />

                 <Button className={props.filter == 'completed'?'activeButton':''} name={'Completed'} callBack ={()=>onClickFilterHandler('completed')} />
            </div>
        </div>
    );
}
